...
function answerFromData(key, a) {
  ...
  switch (key) {
    case "peak": {
      if (!a.peaks.length) return { text: `Nothing in this session held above your baseline long enough to count as a focused stretch, and it stayed fairly level throughout.`, sources: [src] };
      const p = a.peaks[0];
      return {
    ...
    case "coverage":
      return {
        text: `${pct(a.coverage)} of this session had a signal worth trusting (${a.usableRows.toLocaleString()} of ${a.rows.toLocaleString()} readings). ` +
              (a.coverage >= 0.75 ? `That is solid, so the rest of the numbers can be taken at face value.` : `That's low enough that everything else should be treated as provisional. It usually means the electrodes weren't reaching your scalp.`),
        sources: [src],
      };
    case "hour":
      ...
      return {
        text: `In this session your strongest hour was ${String(a.bestHour.hour).padStart(2, "0")}:00, averaging ${a.bestHour.meanFocus.toFixed(2)}` +
              ...
              `. One session is not a pattern though, and that needs a week or two before it means anything.`,
        sources: [src],
      };
    case "bands": {
      const b = a.bands;
    ...
    case "overall":
      return {
        text: `This session ran ${fmtClock(a.startMs)} to ${fmtClock(a.endMs)}, ${fmtDuration(a.recordedMs)} recorded, ${pct(a.coverage)} usable. Focus sat around ${a.focus.p50.toFixed(2)} (normally ${a.focus.p10.toFixed(2)} to ${a.focus.p90.toFixed(2)}), calm around ${a.calm.p50.toFixed(2)}. ${a.peaks.length} focused stretch${a.peaks.length === 1 ? "" : "es"} and ${a.slumps.length} dip${a.slumps.length === 1 ? "" : "s"} stood out. The full write-up is on the Debrief tab.`,
        sources: [src],
      };
    default: return null;
  }
...
export function ask(question, { index, analysis = null, shaping = null, adaptive = true } = {}) {
  ...
  // Session questions first, since they are more specific than anything in the notes.
  for (const p of DATA_PATTERNS) {
    if (p.re.test(q)) {
      ...
      if (!analysis) {
        return {
          text: "That is a question about your own data, but no session is loaded yet. Load a recording and ask me again.",
  ...
  if (!hits.length) {
    return {
      text: "I can't find anything about that in the project notes, and I'd rather say so than guess. The notes cover the headset and the bands, reading your own data, running the app, and the safety and privacy rules. If it is about your session, load a recording first.",
  ...
  if (depth === "short") {
    text = shorten(text, 3);
    text += "\n\nThat is the short version. Ask me to go deeper and I'll give you the whole thing.";
